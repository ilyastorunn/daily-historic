export interface FunctionSearchRecordPage {
  displayTitle?: string;
...
export interface FunctionSearchRecordEvent {
  eventId: string;
  year?: number;
  text?: string;
  summary?: string;
  ...
  relatedPages?: FunctionSearchRecordPage[];
  updatedAt?: unknown;
}

export interface FunctionAlgoliaSearchRecord {
  objectID: string;
  eventId: string;
  title: string;
  summary: string;
...
const resolveImageUrl = (page?: FunctionSearchRecordPage) => {
  if (!page) {
    return undefined;
  }
...
const buildSearchableText = (event: FunctionSearchRecordEvent) => {
  const buffer: string[] = [];
  const pages = Array.isArray(event.relatedPages) ? event.relatedPages : [];
...
export const toAlgoliaSearchRecord = (
  event: FunctionSearchRecordEvent
): FunctionAlgoliaSearchRecord | null => {
  if (!event.eventId) {
    return null;
  }
