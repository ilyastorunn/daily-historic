...
```bash
TOKEN=$(grep ADMIN_SERVICE_TOKEN /Users/ilyastorun/.blitz/projects/daily-historic/.dev.vars | cut -d= -f2)
...
Edit `teenybase.ts` to add or modify tables, then:
```bash
npm run generate:backend   # generate migration SQL
npm run migrate:backend -- -y  # apply migrations
```
